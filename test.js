...
const priceParser = async () => {
    ...
    // Map basePrices to a more readable format
     basePrices = {
      ...
      chinaPremiums:basePrices["CHINA_PREMIUMS"]||{}
    };
...
const runTimePriceUpdateQuery = (basePriceData) => {
  ...
  let chinaPremiumKeys = Object.keys(basePriceData.chinaPremiums).map(Number).sort((a, b) => a - b);
  const decidePremium = () => {
  const generatePremiumConditions = (priceField) => {
      return chinaPremiumKeys.map((key, index) => {
        let nextKey = chinaPremiumKeys[index + 1];
        if (index == 0) {
          return { case: { $lte: [priceField, key] }, then: basePriceData.chinaPremiums[key] };
        }
        return {
          case: { $and: [{ $gt: [priceField, chinaPremiumKeys[index-1]] }, { $lte: [priceField, key] }] },
          then: basePriceData.chinaPremiums[key]
        };
      }).concat([
        {
          // If priceField is greater than the last key, assign the last key itself
          case: { $gt: [priceField, chinaPremiumKeys[chinaPremiumKeys.length - 1]] },
          then: basePriceData.chinaPremiums[chinaPremiumKeys[chinaPremiumKeys.length - 1]]
        }
      ]);
    };
        ...
        premiumPercentage: {
          $switch: {
            branches: [
              ...
              // China Logic with dynamic premium levels
              {
                ...
                then: {
                  $cond: {
                    ...
                    then: "$sellerDetail.premiumPercentage",
                      ...
                      $switch: {
                        branches: generatePremiumConditions("$salePrice"),
                        default: 0 // Default premium if no match found
                      }
                    }
                  }
                }
            ...
            default: 0 // Default for other origins
          }
        },
        // Sale Price Premium Calculation for China
        salePricePremium: {
          $cond: {
            if: { $eq: [ "$origin", "china"] },
            then: {
  ...
  const calculatePrice = (priceField, forex, vat, shipment,premiumPercent) => ({
    $add: [
      { $ifNull: [`$${priceField}`, 0] },
      { $multiply: [{ $ifNull: [`$${priceField}`, 0] }, { $divide: [premiumPercent, 100] }] },
        ...
        regularPrice: {
          $switch: {
            branches: [
              {
                case: { $eq: ["$origin", "pak"] },
                then: calculatePrice("regularPrice", basePriceData.pakForex, basePriceData.pakVat, basePriceData.pakShipment,"$premiumPercentage")
              },
              {
                case: { $eq: ["$origin", "ksa"] },
                then: calculatePrice("regularPrice", basePriceData.ksaForex, basePriceData.ksaVat, basePriceData.ksaShipment,"$premiumPercentage")
              },
              {
                case: { $eq: ["$origin", "china"] },
                then: calculatePrice("regularPrice", basePriceData.chinaForex, basePriceData.chinaVat, basePriceData.chinaShipment,"$premiumPercentage")
              }
            ],
        ...
        salePrice: {
          $switch: {
            branches: [
              {
                case: { $eq: ["$origin", "pak"] },
                then: calculatePrice("salePrice", basePriceData.pakForex, basePriceData.pakVat, basePriceData.pakShipment,"$premiumPercentage")
              },
              {
                case: { $eq: ["$origin", "ksa"] },
                then: calculatePrice("salePrice", basePriceData.ksaForex, basePriceData.ksaVat, basePriceData.ksaShipment,"$premiumPercentage")
              },
              {
                case: { $eq: ["$origin", "china"] },
                then: calculatePrice("salePrice", basePriceData.chinaForex, basePriceData.chinaVat, basePriceData.chinaShipment,"$salePricePremium")
              }
            ],
